...
import { ModelProvider, MODEL_DISPLAY_NAMES, SUPPORTED_PROVIDERS } from "@extrascan/shared/types";
import { useState, useEffect } from "react";
import { getStoredApiKeys, setStoredApiKey } from "../utils/storage";
...
export default function Settings() {
    ...
    const renderProviderInput = (provider: ModelProvider, disabled = false) => (
        <div key={provider} className="space-y-2">
            <label className="block text-xs font-medium text-gray-700">{MODEL_DISPLAY_NAMES[provider]}</label>
            ...
            {errors[provider] && <p className="mt-[2px] text-xs text-red-600">{errors[provider]}</p>}
        </div>
    );
        ...
        <div className="w-full flex flex-col">
            <div className="p-4 space-y-4 flex-1 overflow-auto">
                <div className="space-y-2">
                    <h2 className="text-sm font-semibold text-gray-900">Available Models</h2>
                ...
                <div className="space-y-2">
                    ...
                    {COMING_SOON_PROVIDERS.map((provider) => renderProviderInput(provider, true))}
                </div>
            </div>
        </div>
    );
