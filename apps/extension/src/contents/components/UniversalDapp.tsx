...
} from "@extrascan/shared/components"
    ...
    <div className="w-full">
      {isProxy && (
        <div className="mb-4 bg-cyan-800 bg-opacity-20 p-4 rounded-md flex gap-2 items-center">
          <Icon
            icon="lets-icons:info-duotone"
            className="h-5 w-5 text-cyan-800"
          />
          <p className="text-sm">
            This is a proxy contract. The implementation address is at{" "}
            {implementationAddress}.
          </p>
        </div>
      )}
      <div className="flex gap-x-8 gap-y-2 flex-wrap">
        ...
        <div className="flex gap-4">
          ...
          <span className="text-cyan-800">
            {deployer ? shortenAddress(deployer) : "Unknown"}
          </span>
        </div>
        <div className="flex gap-4">
      ...
      {!isVerified && (
        <p className="mt-4 text-sm text-gray-500">
          The ABI of this smart contract was extrapolated from the bytecode, as
          such, some functions may not be present or accurately extrapolated.
          Associated with each function is a confidence score (from 0 - 1),
          which indicates the likelihood of the function being accurate.
        </p>
      )}
